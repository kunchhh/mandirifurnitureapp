...
class _LampScreenState extends State<LampScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    /* List product Lamp categories */
  Future<List<Products>> lampCategories() async {
    List<Products> allLampCategories = [];

    try {
      var res = await http.post(Uri.parse(api.lamp));

      if (res.statusCode == 200) {
        var responseBodyOfLampCategories = jsonDecode(res.body);
        if (responseBodyOfLampCategories["success"] == true) {
          (responseBodyOfLampCategories["productsData"] as List)
              .forEach((eachRecord) {
            allLampCategories.add(Products.fromJson(eachRecord));
          });
        }
      } else {
    ...
    return allLampCategories;
  }
  /* List product Lamp categoriest end */

  /* Widget otherProducts*/
  Widget otherProducts(context) {
    return FutureBuilder(
      future: lampCategories(),

...
class _ChairScreenState extends State<ChairScreen> {
  ...
  /* List product Chair categories */
  Future<List<Products>> chairCategories() async {
    List<Products> allChairCategories = [];

    try {
      var res = await http.post(Uri.parse(api.chair));

      if (res.statusCode == 200) {
        var responseBodyOfChairCategories = jsonDecode(res.body);
        if (responseBodyOfChairCategories["success"] == true) {
          (responseBodyOfChairCategories["productsData"] as List)
              .forEach((eachRecord) {
            allChairCategories.add(Products.fromJson(eachRecord));
          });
        }
      } else {
    ...
    return allChairCategories;
  }
  /* List product Chair categoriest end */

  /* Widget otherProducts*/
  Widget otherProducts(context) {
    return FutureBuilder(
      future: chairCategories(),

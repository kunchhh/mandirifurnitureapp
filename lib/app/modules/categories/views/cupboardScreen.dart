...
class _CupboardScreenState extends State<CupboardScreen> {
  ...
  /* List product Cupboard categories */
  Future<List<Products>> cupboardCategories() async {
    List<Products> allCupboardCategories = [];

    try {
      var res = await http.post(Uri.parse(api.cupboard));

      if (res.statusCode == 200) {
        var responseBodyOfCupboardCategories = jsonDecode(res.body);
        if (responseBodyOfCupboardCategories["success"] == true) {
          (responseBodyOfCupboardCategories["productsData"] as List)
              .forEach((eachRecord) {
            allCupboardCategories.add(Products.fromJson(eachRecord));
          });
        }
      } else {
    ...
    return allCupboardCategories;
  }
  /* List product Cupboard categoriest end */

  /* Widget otherProducts*/
  Widget otherProducts(context) {
    return FutureBuilder(
      future: cupboardCategories(),

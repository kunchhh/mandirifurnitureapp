...
class KitchenSetScreen extends StatefulWidget {
  const KitchenSetScreen({super.key});

  @override
  State<KitchenSetScreen> createState() => _KitchenSetScreenState();
}

class _KitchenSetScreenState extends State<KitchenSetScreen> {
  @override
  ...
  /* List product KitchenSet categories */
  Future<List<Products>> kitchenSetCategories() async {
    List<Products> allKitchenSetCategories = [];

    try {
      var res = await http.post(Uri.parse(api.kitchen));

      if (res.statusCode == 200) {
        var responseBodyOfKitchenSetCategories = jsonDecode(res.body);
        if (responseBodyOfKitchenSetCategories["success"] == true) {
          (responseBodyOfKitchenSetCategories["productsData"] as List)
              .forEach((eachRecord) {
            allKitchenSetCategories.add(Products.fromJson(eachRecord));
          });
        }
      } else {
    ...
    return allKitchenSetCategories;
  }
  /* List product KitchenSet categoriest end */

  /* Widget otherProducts*/
  Widget otherProducts(context) {
    return FutureBuilder(
      future: kitchenSetCategories(),

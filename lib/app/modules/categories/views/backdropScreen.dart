...
class _BackdropScreenState extends State<BackdropScreen> {
  ...
  /* List product Backdrop categories */
  Future<List<Products>> backdropCategories() async {
    List<Products> allBackdropCategories = [];

    try {
      var res = await http.post(Uri.parse(api.backdrop));

      if (res.statusCode == 200) {
        var responseBodyOfBackdropCategories = jsonDecode(res.body);
        if (responseBodyOfBackdropCategories["success"] == true) {
          (responseBodyOfBackdropCategories["productsData"] as List)
              .forEach((eachRecord) {
            allBackdropCategories.add(Products.fromJson(eachRecord));
          });
        }
      } else {
    ...
    return allBackdropCategories;
  }
  /* List product Backdrop categoriest end */

  /* Widget otherProducts*/
  Widget otherProducts(context) {
    return FutureBuilder(
      future: backdropCategories(),
